...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
	<h1 class="h3 mb-0 text-gray-200"><?= __tr('__languageName__ Language Translations (__translationsCount__)', [
        '__languageName__' => $languageInfo['name'],
        '__translationsCount__' => $translations->count()
    ]) ?></h1>

    <div class="btn-group" role="group" >
        <a class="btn btn-light btn-sm" href="<?= route('manage.translations.languages') ?>" title="<?= __tr('Back to languages') ?>"><i class="fa fa-arrow-left"></i> <?= __tr('Back to languages') ?></a>
    	<a class="btn btn-secondary btn-sm lw-ajax-link-action" href="<?= route('manage.translations.scan', [
        	'languageId' => $languageInfo['id']
    	]) ?>" title="<?= __tr('Re-Scan') ?>"><i class="fa fa-sync-alt"></i> <?= __tr('Re-Scan') ?></a>
        <a class="btn btn-light btn-sm" href="#autoTranslationDialog" data-toggle="modal" data-target="#autoTranslationDialog">
...
<div class="modal fade" id="autoTranslationDialog" tabindex="-1" role="dialog" aria-labelledby="autoTranslationLabel" aria-hidden="true">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
                ...
                <div class="mt-2">
                    <h5><?= __tr('Step 2') ?></h5>
                    <p>
                        <?= __tr('Once downloaded file, Go to __googleSpreadSheetAnchorTag__ and Open the exported spreadsheet using upload.', [
                        '__googleSpreadSheetAnchorTag__' => '<a href="https://docs.google.com/spreadsheets">
                ...
                <div class="mt-2">
                     <h5><?= __tr('Step 4') ?></h5>
                    <?= __tr('Now it translate all your string automatically, Now just export using __downloadFunctionPath__ and Drag & Drop or Browse your excel file to process the translations below.', [
				...
				<div class="col-lg-9 mt-2">
					<input type="file" 
                        ...
                        data-allowed-media='<?= getMediaRestriction('language') ?>'
						data-action="<?= route('manage.translations.import', [ 'languageId' => $languageInfo['id'] ]) ?>"
						data-label-idle="<span class='filepond--label-action'><?= __tr('Import & Process') ?></span>"
                        data-callback="afterSuccessfullyUploaded">
				</div>
			</div>
 ...
 <div class="row">
	<div class="col-xl-12 mb-4 <?= $languageInfo['is_rtl'] ? 'lw-lang-direction-rtl' : 'lw-lang-direction-ltr' ?>">
			...
			<div class="card-body">
                <form class="row lw-ajax-form lw-form" method="post" action="<?= route('manage.translations.update') ?>" data-show-processing="true">
                    <div class="input-group mb-3">
                        ...
                        <button class="btn btn-outline-secondary lw-auto-translate-action" type="button" title="<?= __tr('Google Auto Translate') ?>"><i class="fa fa-language"></i> <?= __tr('Auto Translate') ?></button>
...
<script>
    ...
    function afterSuccessfullyUploaded(responseData) {
