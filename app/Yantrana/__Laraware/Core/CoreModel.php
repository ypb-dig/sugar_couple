...
/*
 * __Laraware 1.x.x - Core Model - 1.6.4 - 01 APR 2020
 *
...
abstract class CoreModel extends Eloquent
{
     ...
     * @return string.
     *-------------------------------------------------------- */
    public static function bunchInsertUpdate(array $data, string $index = null)
    {
        // verify data
        ...
        $existingResult = $instance->select($index)->whereIn($index, $dataToUpdateIndexes)->get();
        // get indexes only
        ...
        return DB::transaction(function () use ($dataToInsert, $dataToUpdate, $dataToInsertWithIndex, $index, $instance) {
            ...
            if (\__isEmpty($dataToUpdate) === false) {
                if ($instance->batchUpdate($dataToUpdate, $index)) {
                    $updateCount++;
                }
            }
